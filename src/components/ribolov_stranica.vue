...
<template>
  <div id="app">
    ...
    <h2 class="naziv_aktivnosti">Ribolov u slatkim vodama</h2>
...
export default {
  ...
  methods: {
    async ucitajDestinacije() {
      try {
        ...
        this.destinacije = destinacijeSnapshot.docs
          .filter(doc => doc.data().izvor === 'ribolov') 
          .map(async doc => {
    ...
    prikaziDetalje(id) {
      this.$router.push({ name: 'ribolov_destinacija', params: { destinacijaId: id } });
    },
    usmjeri_pocetna() {
      this.$router.push('/');
    },
